...
import { CaptionPluginSettings } from './settingsTypes'

// 兼容存根：旧版验证函数（始终返回 true）
function isValidNumberingStyleString(_s: string): boolean {
  return false
}
function isValidSeparator(_s: string): boolean {
  return false
}
...
export function updateSettingsFromFrontMatterFormatPart(part: string, settings: CaptionPluginSettings): CaptionPluginSettings {
  // Parse the separator
